...
const verificationCodes = new Map(); // email → { userData, code, expires }
...
db.serialize(() => {
  ...
  db.get("SELECT COUNT(*) as count FROM products", (err, row) => {
    if (row && row.count === 0) {
      db.run(`INSERT INTO products (name, brand, category, description, price, pack_qty, discounts) VALUES
        ('Тарелка обеденная', 'BONITA', 'Керамика', 'Белая керамика, матовая', 450, 48, '[{"from":2,"percent":5},{"from":5,"percent":10},{"from":10,"percent":15}]')`);
      db.run(`INSERT INTO products (name, brand, category, description, price, pack_qty, discounts) VALUES
        ('Пиала чайная', 'BONITA', 'Керамика', 'Рельефный узор, айвори', 320, 36, '[{"from":3,"percent":7},{"from":6,"percent":12}]')`);
      db.run(`INSERT INTO products (name, brand, category, description, price, pack_type, pack_qty, discounts) VALUES
        ('Набор бокалов', 'VOCO HOME', 'Стекло', '6 предметов, боросиликат', 2800, 'set', 1, '[{"from":5,"percent":8},{"from":20,"percent":15}]')`);
      db.run(`INSERT INTO products (name, brand, category, description, price, pack_qty, discounts) VALUES
        ('Салатник глубокий', 'BONITA', 'Керамика', 'Матовое покрытие, синий', 680, 24, '[{"from":3,"percent":6},{"from":8,"percent":12}]')`);
...
app.post('/api/register', async (req, res) => {
  ...
  db.get(checkSql, checkParams, async (err, existing) => {
    ...
    try {
      ...
      // No email → create account directly (no email to verify)
      if (!email) {
        db.run(
      ...
      try {
        await mailer.sendMail({
          from: `"Bonita" <${GMAIL_USER}>`,
          to: email,
          subject: 'Подтверждение регистрации — Bonita',
          text: `Ваш код подтверждения: ${code}\n\nКод действителен 15 минут.`,
          html: `
            <div style="font-family:sans-serif;max-width:420px;margin:0 auto;color:#1a1a1a;">
              <h2 style="font-size:22px;font-weight:300;border-bottom:1px solid #e5e5e5;padding-bottom:12px;">
                Подтверждение регистрации
              </h2>
              <p style="color:#555;font-size:14px;">Введите этот код для активации аккаунта:</p>
              <div style="font-size:36px;font-weight:700;letter-spacing:0.3em;color:#c9a84c;
                          background:#faf7f0;border:1px solid #e8d5a3;padding:16px 24px;
        ...
        res.status(500).json({ error: 'Не удалось отправить письмо подтверждения. Проверьте email.' });
      }
    } catch (e) {
      ...
      await mailer.sendMail({
        ...
        html: `
          <div style="font-family:sans-serif;max-width:420px;margin:0 auto;color:#1a1a1a;">
            ...
            <p style="color:#888;font-size:12px;">Код действителен 15 минут.</p>
            <p style="color:#aaa;font-size:11px;">Если вы не запрашивали сброс пароля, проигнорируйте это письмо.</p>
          </div>`
      });
  ...
  db.get(sql, params,
    (err, order) => {
      ...
      try {
        ...
        doc.font(FONT_B).fontSize(18).fillColor(DARK)
           .text(`Счёт #${order.id}`, 0, 50, { align: 'right' });
        ...
        doc.font(FONT_B).fontSize(12).fillColor(DARK)
           .text(`${order.name || ''} ${order.surname || ''}`, 50, 145);
        doc.font(FONT).fontSize(9).fillColor(MUTED);
        ...
        y += 12;
        doc.font(FONT_B).fontSize(11).fillColor(DARK)
           .text('ИТОГО К ОПЛАТЕ:', cols.name, y)
           .fillColor(GOLD)
           .text(`${(order.total || 0).toLocaleString('ru')} ₸`, cols.total, y, { width: 75, align: 'right' });
...
app.get('/api/banners/settings', (req, res) => {
  res.json(readBannerSettings());
});
...
app.post('/api/banners/settings', adminAuth, (req, res) => {
  ...
  const settings = { interval };
  fs.writeFileSync(settingsFile, JSON.stringify(settings));
  res.json({ message: 'Сохранено', ...settings });
});
